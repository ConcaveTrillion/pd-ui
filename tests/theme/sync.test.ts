...
function buildTmpWorkspace(
  ...
  const prefix = withSpaceInPath
    ? resolve(tmpdir(), 'pd ui sync test ')
    : resolve(tmpdir(), 'pdomain-ui-sync-test-');
...
/** Run the sync script inside a given pdomain-ui directory, using --force to skip git guard */
function runSync(tmpRoot: string, extraArgs: string[] = []): ReturnType<typeof spawnSync> {
  ...
  //
  // Simpler approach: write a wrapper script into the tmp pdomain-ui/scripts/
...
describe('sync-design-system.mjs', () => {
  it('syncs pdomain-ui/theme/ to docs/design-system/ (--force skips git guard)', () => {
    const result = runSync(tmpRoot, ['--force']);

    expect(result.status, `script stderr: ${String(result.stderr ?? '')}`).toBe(0);

    // docs/design-system/ should now match pdomain-ui/theme/
    const docTokens = readFileSync(
      resolve(tmpRoot, 'docs', 'design-system', 'tokens.css'),
      'utf-8',
    );
    const pdTokens = readFileSync(resolve(tmpRoot, 'pdomain-ui', 'theme', 'tokens.css'), 'utf-8');
  ...
  it('--check exits non-zero when diff exists', () => {
    // Ensure docs differ from pdomain-ui (they do from beforeEach)
