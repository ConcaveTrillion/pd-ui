...
describe('PageImageCanvas — pointer-events stage callbacks (#4)', () => {
  ...
  it('accepts onStagePointerDown prop without TypeScript error', () => {
    ...
    expect(() => {
      render(
        <PageImageCanvas src="test.png" page={PAGE} words={WORDS} onStagePointerDown={onDown} />,
      );
    }).not.toThrow();
  });
  ...
  it('fires onStagePointerMove with (e, CoordContext) when stage fires mousemove', () => {
    const onMove = vi.fn();
    render(
      <PageImageCanvas src="test.png" page={PAGE} words={WORDS} onStagePointerMove={onMove} />,
    );
    act(() => {
  ...
  it('fires onStagePointerUp with (e, CoordContext) when stage fires mouseup', () => {
    const onUp = vi.fn();
    render(<PageImageCanvas src="test.png" page={PAGE} words={WORDS} onStagePointerUp={onUp} />);
    act(() => {
