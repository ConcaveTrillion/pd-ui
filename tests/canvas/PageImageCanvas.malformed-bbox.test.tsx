...
describe('PageImageCanvas — malformed bbox words (issue #22)', () => {
  it('does not crash when words array contains a word with null bounding_box', () => {
    expect(() => {
      render(
        <PageImageCanvas src="/img.png" page={page} words={[nullBBoxWord]} />
      )
    }).not.toThrow()
  })
  ...
  it('does not crash when words array contains a word with undefined bounding_box', () => {
    expect(() => {
      render(
        <PageImageCanvas src="/img.png" page={page} words={[undefinedBBoxWord]} />
      )
    }).not.toThrow()
  })
  ...
  it('does not crash when a word has NaN coordinates in bounding_box', () => {
    expect(() => {
      render(
        <PageImageCanvas src="/img.png" page={page} words={[nanCoordWord]} />
      )
    }).not.toThrow()
  })
  ...
  it('does not crash when a word has Infinity coordinates in bounding_box', () => {
    expect(() => {
      render(
        <PageImageCanvas src="/img.png" page={page} words={[infinityCoordWord]} />
      )
    }).not.toThrow()
  })
  ...
  it('does not crash when a word has -Infinity coordinates in bounding_box', () => {
    expect(() => {
      render(
        <PageImageCanvas src="/img.png" page={page} words={[negativeInfinityCoordWord]} />
      )
    }).not.toThrow()
  })
  ...
  it('does not crash when a word is missing top_left', () => {
    expect(() => {
      render(
        <PageImageCanvas src="/img.png" page={page} words={[missingTopLeftWord]} />
      )
    }).not.toThrow()
  })
  ...
  it('does not crash when a word is missing bottom_right', () => {
    expect(() => {
      render(
        <PageImageCanvas src="/img.png" page={page} words={[missingBottomRightWord]} />
      )
    }).not.toThrow()
  })
  ...
  it('getWordId default fallback returns a stable string for invalid bbox', () => {
    ...
    render(
      <PageImageCanvas
        src="/img.png"
        page={page}
        words={[makeValidWord(5, 5, 10, 10, 'ok')]}
      >
        {{
          overlay: () => {
            didRender = true
  ...
  it('mixed batch: malformed words are excluded from marquee selection result', () => {
    ...
    const allWords = [
      nullBBoxWord,
      nanCoordWord,
      makeValidWord(0, 0, 100, 100, 'selectable'),
    ]
    expect(() => {
      render(
        <PageImageCanvas src="/img.png" page={page} words={allWords} />
...
describe('isValidBBox', () => {
  it('returns true for a well-formed bbox', () => {
    expect(
      isValidBBox({ top_left: { x: 0, y: 0 }, bottom_right: { x: 10, y: 10 } })
    ).toBe(true)
  })
  ...
  it('returns false when top_left.x is NaN', () => {
    expect(
      isValidBBox({ top_left: { x: NaN, y: 0 }, bottom_right: { x: 10, y: 10 } })
    ).toBe(false)
  })

  it('returns false when top_left.y is NaN', () => {
    expect(
      isValidBBox({ top_left: { x: 0, y: NaN }, bottom_right: { x: 10, y: 10 } })
    ).toBe(false)
  })

  it('returns false when bottom_right.x is NaN', () => {
    expect(
      isValidBBox({ top_left: { x: 0, y: 0 }, bottom_right: { x: NaN, y: 10 } })
    ).toBe(false)
  })

  it('returns false when bottom_right.y is NaN', () => {
    expect(
      isValidBBox({ top_left: { x: 0, y: 0 }, bottom_right: { x: 10, y: NaN } })
    ).toBe(false)
  })

  it('returns false when top_left.x is Infinity', () => {
    expect(
      isValidBBox({ top_left: { x: Infinity, y: 0 }, bottom_right: { x: 10, y: 10 } })
    ).toBe(false)
  })

  it('returns false when top_left.y is -Infinity', () => {
    expect(
      isValidBBox({ top_left: { x: 0, y: -Infinity }, bottom_right: { x: 10, y: 10 } })
    ).toBe(false)
  })
  ...
  it('returns true for bbox with negative coordinates (valid but inverted)', () => {
    ...
    expect(
      isValidBBox({ top_left: { x: -10, y: -20 }, bottom_right: { x: -1, y: -5 } })
    ).toBe(true)
  })
...
describe('bboxToRect — extended validation (issue #22)', () => {
  ...
  it('returns null when top_left.x is NaN', () => {
    expect(
      bboxToRect({ top_left: { x: NaN, y: 0 }, bottom_right: { x: 10, y: 10 } })
    ).toBeNull()
  })
