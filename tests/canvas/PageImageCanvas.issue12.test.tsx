...
describe('PageImageCanvas — imageNodeRef (#12)', () => {
  it('accepts onImageNodeReady prop without type errors', () => {
    ...
    expect(() => {
      render(
        <PageImageCanvas src="test.png" page={PAGE} words={WORDS} onImageNodeReady={onReady} />,
      );
    }).not.toThrow();
  });
  ...
  it('renders the image-viewport when onImageNodeReady is provided', () => {
    const onReady = vi.fn();
    render(<PageImageCanvas src="test.png" page={PAGE} words={WORDS} onImageNodeReady={onReady} />);
  ...
  it('image layer renders an Image node (not just a Rect fill pattern)', () => {
    ...
    const layers = Array.from(document.querySelectorAll('[data-layer-name="image"]'));
