...
const Sparkline = ({ series, w = 220, h = 36 }) => {
  ...
  const tickXs = [0, 0.5, 1].map(p => pad + p * (w - pad * 2));
  return (
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <span className="mono" style={{ fontSize: 10.5, color: 'var(--ink-4)' }}>
          {c.crossPage
            ? `${c.crossPage.fromPage} → ${c.crossPage.toPage} · conf ${c.confidence.toFixed(2)}`
            : `${c.pageId} · L${c.line} · conf ${c.confidence.toFixed(2)}`}
        </span>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span className="mono" style={{ fontSize: 11, color: 'var(--ink-4)' }}>line {c.line} · conf {c.confidence.toFixed(2)}</span>
        <div style={{ flex: 1 }} />
...
const AutoJoinedRow = ({ w, expanded, selected }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        display: 'grid',
        gridTemplateColumns: '24px 1fr 64px 200px 1fr 80px 220px',
        gap: 12, alignItems: 'center',
        padding: '10px 16px',
      }}>
        ...
        <NgramLink c={{ head: w.head, tail: w.tail }} />
