...
const StageContextStrip = ({
  currentStage = 'source',
  flaggedAtStage = 47, dirtyAtStage = 167,
  jumpOpen,
  variant = 'project',     // 'project' | 'page'
  pageStageStatus,         // { stageId: 'clean' | 'dirty' | 'running' | 'failed' | 'notrun' }
  pageId,                  // e.g. 'p012'
  pageLabel,               // human-readable inspecting label override
}) => {
  ...
  const pageStatusFor = (s) => {
    if (variant !== 'page') return null;
    return (pageStageStatus && pageStageStatus[s]) || (s === currentStage ? 'running' : 'notrun');
  };
  const curPageStatus = variant === 'page' ? pageStatusFor(currentStage) : null;
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: '0 0 auto', minWidth: variant === 'page' ? 320 : 280 }}>
        <div style={{ fontSize: 10.5, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--ink-4)' }}>
          {variant === 'page' ? 'Inspecting' : 'Stage'}
        </div>
        <div style={{
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 3, minWidth: 0, overflow: 'hidden' }}>
        {STAGE_DEFS.map((s, i) => {
          const st = variant === 'page' ? pageStatusFor(s.id) : state(i);
          const isCur = i === idx;
          return (
                ...
                <div title={`${i + 1}. ${s.id}`} style={{
                  ...
                  <span style={{
                    ...
                    background:
                      ...
                      st === 'failed' ? 'var(--mismatch)' :
                      'var(--ink-4)',
                    opacity: st === 'clean' || st === 'dirty' || st === 'failed' || st === 'running' ? 1 : 0.55,
                    border:
                      st === 'clean' ? '1px solid color-mix(in oklab, var(--exact) 60%, transparent)' :
                      st === 'dirty' ? '1px solid color-mix(in oklab, var(--fuzzy) 60%, transparent)' :
                      st === 'failed' ? '1px solid color-mix(in oklab, var(--mismatch) 60%, transparent)' :
      ...
      {variant === 'page' ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 0 auto' }}>
          <span style={{
            width: 8, height: 8, borderRadius: 99,
            background:
              curPageStatus === 'clean' ? 'var(--exact)' :
              curPageStatus === 'dirty' ? 'var(--fuzzy)' :
              curPageStatus === 'failed' ? 'var(--mismatch)' :
              curPageStatus === 'running' ? 'var(--ocr)' :
              'var(--ink-4)',
          }} />
          <span className="mono" style={{ fontSize: 11.5, color: 'var(--ink-2)' }}>
            {pageId || 'page'} · {curPageStatus || 'not run'}
          </span>
        </div>
      ) : (
