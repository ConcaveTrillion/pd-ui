...
| `StageControlsPanel` | `:457-668` | Left-drawer container that hosts per-stage control sub-components. Slot-based: accepts a `controlsSlot` render prop for per-stage controls (`ThresholdControls`, `GrayscaleControls`, `DeskewControls`, `InitialCropControls`, `OcrControls`). Props: `controlsSlot`, `inheritance`, `onRevert`, `onSave`. |
| `HierarchyTreePanel` | `:1296-1414` | Right-drawer tree panel: block/paragraph/line/word hierarchy with `TreeRow` (expand/collapse + type badge + selection). Props: `tree`, `selectedId`, `onSelect`, `onTypeChange`. |
| `BlockTypePickerPanel` | `:1440-1534` | Right-drawer type picker: grid of block types (`TypeGrid`) for reassigning selected block. Props: `selectedType`, `onSelect`. |
| `PageAttributesPanel` | `:1538-1622` | Right-drawer full attribute editor (full expansion of `PageAttributesBar`). Props: `attrs`, `onChange`. |
...
| `TextReviewPane` | `:711-785` | Collapsible bottom pane (280px open / 44px collapsed). Props: `text`, `open`, `onOpenChange`. |
