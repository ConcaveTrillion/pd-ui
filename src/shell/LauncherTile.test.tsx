...
function wrapWithContext(
  ...
  return (
    <SuiteSiblingsContext.Provider
      value={{ siblings: [sibling()], launch, loading: false }}
    >
      {children}
    </SuiteSiblingsContext.Provider>
  );
...
describe('LauncherTile (#159)', () => {
  it('renders sibling displayName', () => {
    const launch = vi.fn((): Promise<LaunchResult> => Promise.resolve({ kind: 'opened' as const, url: 'http://localhost:3001' }));
  ...
  it('calls launch(id) when clicked', async () => {
    const launch = vi.fn((): Promise<LaunchResult> => Promise.resolve({
      kind: 'opened',
