 ...
 */
export const PageViewer = React.forwardRef<HTMLDivElement, PageViewerProps>(
  function PageViewer(
    {
      page,
      mode,
    ...
    const imageSrc =
      mode === 'before' ? page.beforeImageUrl : page.afterImageUrl;

    const overlayMode = mode === 'split' ? 'split' : 'view';

    const splitProposal =
      mode === 'split' ? { splitX: 0.5 } : undefined;
