...
describe('ThumbGrid', () => {
  it('renders with thumb-grid class', () => {
    const { container } = render(<ThumbGrid items={[]} renderItem={() => <div />} />);
    expect(container.querySelector('.thumb-grid')).toBeTruthy();
  });

  it('renders correct number of items', () => {
    const items = [{ id: 'a' }, { id: 'b' }, { id: 'c' }];
    render(<ThumbGrid items={items} renderItem={(item) => <span key={item.id}>{item.id}</span>} />);
  ...
  it('renders empty state when no items and emptyState provided', () => {
    render(<ThumbGrid items={[]} renderItem={() => <div />} emptyState={<p>No thumbnails</p>} />);
  ...
  it('renders each item inside a thumb-grid__cell wrapper', () => {
    const items = [{ id: 'x' }];
    const { container } = render(
      <ThumbGrid items={items} renderItem={(item) => <span>{item.id}</span>} />,
    );
